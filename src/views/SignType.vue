...
const base64Params = ref('');
const platform = ref('');
const org = ref('');
const repo = ref('');
const signType = ref('corporation');
const transparentDiv = ref({
  height: '',
});
const corpBtTooltip = computed(() => {
  if (isBindCorpCLA.value) {
    return `${$t('signType.corpStep1_1')}${$t('signType.corpBt')}${$t(
      'signType.corpStep1_2'
    )}`;
  } else {
    return $t('signType.not_support_corp');
  }
});
...
const clickIndividualGuide = () => {
  individualGuideIsOpen.value = !individualGuideIsOpen.value;
};
const clickEmpGuide = () => {
  employeeGuideIsOpen.value = !employeeGuideIsOpen.value;
};
const clickCorpGuide = () => {
  corpGuideIsOpen.value = !corpGuideIsOpen.value;
};
const toCorpLogin = () => {
...
const getRepoInfo = () => {
  ...
  if (domain === claConfig.OPENLOOKENG_SIGN_URL) {
    ...
    let urlParams = route.params.params;
